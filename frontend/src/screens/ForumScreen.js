...
const ForumScreen = () => {
    ...
    const handleAskQuestion = async () => {
        const wordCount = askText.trim().split(/\s+/).filter(w => w.length > 0).length;
        
        if (!askCategory || askText.trim().length < 10) {
            Alert.alert('Incomplete', 'Please select a category and enter your question (min 10 characters).');
            return;
        }
