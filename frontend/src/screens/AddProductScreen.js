...
const AddProductScreen = ({ route, navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!price) return Alert.alert('Missing Field', 'Please enter a price.');
    if (!quantity) return Alert.alert('Missing Field', 'Please enter the available quantity.');
    ...
    try {
      ...
      if (editProduct) {
        ...
        Alert.alert(
          ...
          [{ text: 'OK', onPress: () => {
             // Go back but we might need to pop to go to previous screens, or just goBack will return to ProductDetail
             navigation.goBack();
          }}]
        );
      } else {
        ...
        Alert.alert(
          '✅ Product Listed!',
          'Your product has been listed on Govi Mart successfully.',
          [{ text: 'OK', onPress: () => navigation.goBack() }]
        );
      }
    } catch (err) {
      Alert.alert('Error', err.response?.data?.message || 'Could not list product. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.screen} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      {/* Header */}
